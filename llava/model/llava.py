...
class LlavaLlamaModel(LlamaModel):
    ...
    def __init__(self, config: LlamaConfig, mm_vision_tower=None, mm_hidden_size=None):
        ...
        with ContextManagers(deepspeed_zero_init_disabled_context_manager()):
            if hasattr(config, "mm_vision_tower"):
                ...
                # self.vision_tower = AutoModel.from_pretrained(config.mm_vision_tower, trust_remote_code=True)
                pass
    ...
    def initialize_vision_modules(
        ...
        # image_processor = AutoProcessor.from_pretrained(vision_tower, trust_remote_code=True)
        ...
        if not hasattr(self, "vision_tower"):
            ...
            self.vision_tower = CLIPVisionModel.from_pretrained(
                self.config.mm_vision_tower, trust_remote_code=True
            )
        ...
        if (
            not hasattr(self, "mm_projector")
            or self.config.mm_hidden_size != vision_config.hidden_size
            or is_test
        ):
            # breakpoint()
            pre_hidden_size = self.config.mm_hidden_size
            self.config.mm_hidden_size = vision_config.hidden_size
            self.mm_projector = nn.Linear(
                self.config.mm_hidden_size, self.config.hidden_size
            )

            # self.mm_projector = nn.Sequential(nn.Linear(vision_config.hidden_size, pre_hidden_size),
            #                                   self.mm_projector)
                            ...
                            cur_new_input_embeds = torch.cat(
                                (
                                    ...
                                    cur_input_embeds[
                                        ...
                                        + num_patches
                                        + 1 : image_start_token_pos + num_patches + 2
                                    ],
                                    cur_input_embeds[
