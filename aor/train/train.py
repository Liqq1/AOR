...
from aor.train.llava_trainer import LLaVATrainer
...
def _mask_targets(target, tokenized_lens, speakers, tokenizer=None):
...
def preprocess_v1(
    ...
    if is_test:
        conversations[0] = conversations[0] + "ASSISTANT:"
    ...
    for conversation, target in zip(conversations, targets):
        ...
        # assert "cot_bbox" not in conversation, conversation
...
def preprocess(
    ...
    if conversation_lib.default_conversation.version == "v1":
        return preprocess_v1(sources, tokenizer, is_test)
    ...
    for target, source in zip(targets, sources):
        ...
        _mask_targets(target, tokenized_lens, speakers, tokenizer)
...
def train():
    ...
    if model_args.vision_tower is not None:
        if "mpt" in model_args.model_name_or_path:
            ...
            from aor.models.spi_llava import SPILlavaMPTForCausalLM
    ...
    if model_args.vision_tower is not None:
        ...
        if True and len(params_no_grad) > 0:
    ...
    # avoid circle import
    from aor.datasets.data_modules import make_multitask_data_module

    data_module = make_multitask_data_module(tokenizer=tokenizer, data_args=data_args)
    if get_rank() == 0:
        print("train set:", len(data_module["train_dataset"]))
