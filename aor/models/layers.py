    ...
    def _single_shuffle(self, inputs, conv_module):
        ...
        for single_conv_m in conv_module:
            ...
            for fuse_lvl_tuple in self.fuse_lvl_list:
                ...
                fused_inputs.append(
                    torch.cat([remain, from_top, from_down], dim=1).type(dtype)
                )
...
class MLVLROIQueryModule(nn.Module):
    def __init__(self, embed_dims=1024, out_dims=4096, num_levels=3):
        ...
        # s = 16
        s = 14 if os.environ.get("CLIP", False) == "1" else 16
        img_size = 336 if os.environ.get("V15", "0") == "1" else 224
        strids = [s / 8, s / 4, s / 2, s]
        assert len(strids) == num_levels
        bbox_roi_extractor = dict(
            roi_layer=dict(type="RoIAlign", output_size=s, sampling_ratio=2),
    ...
    def forward(self, mlvl_feats, bboxes):
        if mlvl_feats[0].dim() == 3:
            ...
            assert h in [14, 16, 24] and w in [14, 16, 24]
...
class MlvlRoIExtractor(BaseRoIExtractor):
    ...
    def forward(self, feats, rois, roi_scale_factor=None):
        ...
        for img_id, single_img_roi in enumerate(rois):
            ...
            single_img_roi = single_img_roi * self.img_size
